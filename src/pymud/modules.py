...
class ModuleInfo:
                ...
                if issubclass(attr, IConfig) or (attr_name == "Configuration"):
                    try:
                        ...
                        if not reload:
                            self.session.info(
                                Settings.gettext(
                                    "configuration_created", self.name, attr_name
                                )
                            )
                        else:
                            self.session.info(
                                Settings.gettext(
                                    "configuration_recreated", self.name, attr_name
                                )
                            )
                        ...
                        self.session.error(
                            Settings.gettext(
                                "configuration_fail", self.name, attr_name, e
                            )
                        )
    ...
    def load(self):
        ...
        if self._load():
            self.session.info(
                f"{Settings.gettext('entity_module' if self.ismainmodule else 'non_entity_module')} {self.name} {Settings.gettext('load_ok')}"
            )
        else:
            self.session.info(
                f"{Settings.gettext('entity_module' if self.ismainmodule else 'non_entity_module')} {self.name} {Settings.gettext('load_fail')}"
            )
    ...
    def unload(self):
        ...
        self._loaded = False
        self.session.info(
            f"{Settings.gettext('entity_module' if self.ismainmodule else 'non_entity_module')} {self.name} {Settings.gettext('unload_ok')}"
        )
    ...
    def reload(self):
        ...
        self.session.info(
            f"{Settings.gettext('entity_module' if self.ismainmodule else 'non_entity_module')} {self.name} {Settings.gettext('reload_ok')}"
        )
...
class IConfigBase(metaclass=PymudMeta):
    ...
    def __init__(self, session=None, *args, **kwargs):
        from .objects import Alias, GMCPTrigger, Timer, Trigger
        from .session import Session
                ...
                for deco in decorators:
                    if isinstance(deco, PymudDecorator):
                        if deco.type == "alias":
                            ...
                            ali = Alias(
                                self.session, *deco.args, **deco.kwargs, onSuccess=func
                            )
    ...
    def reload(self):
        ...
        if self.modspec and self.modspec.loader:
            ...
            raise FileNotFoundError(
                Settings.gettext("exception_plugin_file_not_found", self._plugin_file)
            )
