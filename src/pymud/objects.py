...
class GMCPTrigger(BaseObject):
    ...
    def __call__(self, value: str) -> Any:
        ...
        if callable(self._onSuccess):
            try:
                if inspect.iscoroutinefunction(self._onSuccess):
                    self.create_task(self._onSuccess(self.id, value, value_exp))
                else:
                    self._onSuccess(self.id, value, value_exp)

                self.event.set(
                    value=BaseObject.State(True, self.id, self.line, self.value)
                )
            ...
            if docallback:
                if self.sync:
                    if state.result == self.SUCCESS:
                        ...
                        if inspect.iscoroutinefunction(self._onFailure):
                            self.create_task(
                        ...
                        if inspect.iscoroutinefunction(self._onTimeout):
                            self.create_task(
...
class Alias(MatchObject):
    ...
    def __init__(self, session, patterns, *args, **kwargs):
        if "patterns" not in kwargs.keys():
...
class Trigger(MatchObject):
    ...
    def __init__(
        self,
        session,
        patterns: Union[str, Union[Tuple[str], List[str]]],
        *args,
        **kwargs,
    ):
