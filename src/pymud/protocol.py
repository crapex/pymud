...
class MudClientProtocol(Protocol):
        ...
        for byte in data:
            ...
            # 状态机为 等待命令，接下来应该收到的字节仅可能包括： WILL/WONT/DO/DONT/SB
                ...
                if byte in (WILL, WONT, DO, DONT):  # 此时，后续选项仅1个字节
                    ...
                    self._state_machine = (
                        "waitsubnegotiation"  # 后续为子协商，以 IAC SE终止
                    )
                elif byte == NOP:  # 空操作 TODO 确认空操作是否为IAC NOP，没有其他
                    self.log.debug("收到服务器NOP指令: IAC NOP")
                    self._state_machine = "normal"
                    # 对NOP信号和GA信号处理相同
                    self.session.go_ahead()
                elif byte == GA:
                    self.log.debug("收到服务器GA指令: IAC GA")
                    self._state_machine = "normal"
                ...
                if byte in _option_name_str.keys():
                    ...
                    if iac_handler and callable(iac_handler):
                        self.log.debug(
                            f"收到IAC选项协商: IAC {name_command(self._iac_command)} {name_option(byte)}, 并传递至处理函数 {iac_handler.__name__}"
                        )
                        iac_handler(self._iac_command)  # 执行IAC协商
                    else:
                        self.log.debug(
                            f"收到不支持(尚未定义处理函数)的IAC协商: IAC {name_command(self._iac_command)} {name_option(byte)}, 将使用默认处理（不接受）"
                        )
                        self._iac_default_handler(self._iac_command, byte)
                    self._state_machine = "normal"  # 状态机恢复到正常状态
                else:
                    self.log.warning(
                        f"收到不识别(不在定义范围内)的IAC协商: IAC {name_command(self._iac_command)} {name_option(byte)}, 将使用默认处理（不接受）"
                    )
                    self._iac_default_handler(self._iac_command, byte)
                    self._state_machine = "normal"  # 状态机恢复到正常状态
                ...
                if byte == SE:  # IAC SE 表示子协商已接收完毕
                    ...
                    if self._iac_sub_neg_option in _option_name_str.keys():
                        ...
                        if iac_subneg_handler and callable(iac_subneg_handler):
                            self.log.debug(
                                f"收到{name_option(self._iac_sub_neg_option)}选项子协商: {self._iac_sub_neg_data}, 并传递至处理函数 {iac_subneg_handler.__name__}"
                            )
                            iac_subneg_handler(self._iac_sub_neg_data)
                        else:
                            self.log.debug(
                                f"收到不支持(尚未定义处理函数)的{name_option(self._iac_sub_neg_option)}选项子协商: {self._iac_sub_neg_data}, 将丢弃数据不处理."
                            )
                else:
    ...
    def _iac_default_handler(self, cmd, option):
        ...
        if cmd in (WILL, WONT):
            ...
            self.log.error(
                f"选项协商进入非正常分支，参考数据: , _iac_default_handler, {cmd}, {option}"
            )
            return

        self.session.write(IAC + ack + option)
        self.log.debug(
            f"使用默认协商处理拒绝了服务器的请求的 IAC {name_command(cmd)} {name_option(option)}, 回复为 IAC {name_command(ack)} {name_option(option)}"
        )
    ...
    def handle_sga(self, cmd):
        ...
        if cmd == WILL:
            if Settings.server["SGA"]:
                ...
                self.log.debug("发送选项协商, 同意抑制GA信号 IAC DO SGA.")
            else:
                self.session.write(IAC + DONT + SGA)
                self.log.debug("发送选项协商, 不同意抑制GA信号 IAC DONT SGA.")

        else:
            self.log.warning(
                f"收到服务器的未处理的SGA协商: IAC {name_command(cmd)} SGA"
            )
    ...
    def handle_echo(self, cmd):
        ...
        if cmd == WILL:
            if Settings.server["ECHO"]:
                ...
                self.log.debug("发送选项协商, 同意ECHO选项协商 IAC DO ECHO.")
            else:
                self.session.write(IAC + DONT + ECHO)
                self.log.debug("发送选项协商, 不同意ECHO选项协商 IAC DONT ECHO.")

        else:
            self.log.warning(
                f"收到服务器的未处理的ECHO协商: IAC {name_command(cmd)} ECHO"
            )
    ...
    def handle_charset(self, cmd):
        ...
        if cmd == WILL:
            ...
            self.log.debug("发送选项协商, 同意CHARSET协商 IAC DO CHARSET.等待子协商")
        elif cmd == WONT:
            nohandle = True
        elif cmd == DO:
            nohandle = True
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的CHARSET协商: IAC {name_command(cmd)} TTYPE"
            )
    ...
    def handle_charset_sb(self, data: bytes):
        ...
        if data[3] == REQUEST:
            ...
            if "utf-8" in charset_list:
                ...
                self.log.debug(
                    '发送CHARSET子协商，同意UTF-8编码 IAC SB ACCEPTED "UTF-8" IAC SE'
                )
                unhandle = False
    ...
    def handle_ttype(self, cmd):
        ...
        if cmd == WILL:
            ...
            self._mtts_index = 0
            self.log.debug(
                "发送选项协商, 同意MTTS(TTYPE)协商 IAC WILL TTYPE.等待子协商"
            )
        elif cmd == DONT:
            nohandle = True
        else:
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的TTYPE/MTTS协商: IAC {name_command(cmd)} TTYPE"
            )
    ...
    def handle_ttype_sb(self, data):
        ...
        if (len(data) == 6) and (data[3] == SEND):
            if self._mtts_index == 0:
                ...
                self._mtts_index += 1
                self.log.debug(
                    f'回复第一次MTTS子协商: IAC SB TTYPE IS "{Settings.__appname__}" IAC SE'
                )
                ...
                self._mtts_index += 1
                self.log.debug(
                    '回复第三次MTTS子协商: IAC SB TTYPE IS "MTTS 783" IAC SE'
                )
            else:
                self.log.warning(
                    f"收到第{self._mtts_index + 1}次(正常为3次)的MTTS子协商, 将不予应答"
                )
        else:
    ...
    def handle_naws(self, cmd):
        ...
        if cmd == WILL:
            ...
            self.log.debug("发送选项协商, 同意NAWS协商 IAC WILL NAWS.")
            # 2. 发送子协商确认尺寸
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的NAWS协商: IAC {name_command(cmd)} NAWS"
            )
    ...
    def handle_mnes(self, cmd):
        ...
        if cmd == WILL:
            ...
            self.log.debug("发送选项协商, 同意MNES协商 IAC WILL MNES. 等待服务器子协商")
        elif cmd == DONT:
            nohandle = True
        else:
            nohandle = True
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的MNES协商: IAC {name_command(cmd)} MNES"
            )
    ...
    def handle_gmcp(self, cmd):
        ...
        if cmd == WILL:
            ...
            if Settings.server["GMCP"]:
                ...
                self.log.debug("发送选项协商, 同意GMPC协商 IAC DO GMCP.")
            else:
                self.session.write(IAC + DONT + GMCP)
                self.log.debug("发送选项协商, 不同意GMPC协商 IAC DONT GMCP.")
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的GMCP协商: IAC {name_command(cmd)} GMCP"
            )
    ...
    def handle_msdp(self, cmd):
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的MSDP协商: IAC {name_command(cmd)} MSDP"
            )
    ...
    def send_msdp_sb(self, cmd: bytes, param: bytes):
        ...
        self.session.write(sbneg)
        self.log.debug(
            f'发送MSDP子协商查询支持的MSDP命令 IAC SB MSDP MSDP_VAR "{cmd.decode(self.encoding)}" MSDP_VAL "{param.decode(self.encoding)}" IAC SE.'
        )
        ...
        for idx in range(3, len(data) - 2):
            ...
            if state_machine == "wait_var":  # 下一个字节为类型 MSDP_VAR
                if byte == MSDP_VAR:
                    ...
                    self.log.warning(
                        f"MSDP状态机错误: 在状态wait_var时收到数据不是MSDP_VAR, 而是{str(byte)}"
                    )
                ...
                if byte == MSDP_VAL:  # MSDP_VAL表示变量名结束，下一个是value了
                    ...
                    self.log.warning(
                        f"MSDP状态机错误: 在状态wait_var_name时收到数据不是MSDP_VAL, 而是{byte}"
                    )
                    # 应该丢弃数据，直接返回 return
                else:
                    var_name.append(byte)
                ...
                if byte == MSDP_ARRAY_OPEN:  # value是一个数组
                    ...
                    self.log.debug(
                        f"收到文本形式的MSDP子协商数据： {current_var} = '{current_val}'"
                    )  # type: ignore
                else:  # value是正常数据
                ...
                if byte == MSDP_ARRAY_CLOSE:
                    ...
                    state_machine = "wait_end"
                    self.log.debug(
                        f"收到数组形式的MSDP子协商数据： {current_var} = '{val_in_array}'"
                    )  # type: ignore
                elif byte == MSDP_VAL:
                ...
                if byte == MSDP_TABLE_CLOSE:
                    ...
                    state_machine = "wait_end"
                    self.log.debug(
                        f"收到表格形式的MSDP子协商数据： {current_var} = '{val_in_table}'"
                    )  # type: ignore
                elif byte == MSDP_VAR:
                    if (
                        len(table_var_name) > 0
                    ):  # 上一个VAL已完成，保存到table，后面继续为VAR
                        val_in_table[table_var_name.decode(self.encoding)] = (
                            table_var_value.decode(self.encoding)
                        )
                    ...
                    if state_machine_table == "wait_table_var":
                        ...
                        self.log.warning(
                            f"MSDP状态机错误: 在状态wait_val_in_table时状态不正确，为{state_machine_table}，收到数据是{byte}"
                        )

        # 更新MSDP_支持的数据到服务器
        self.msdp.update(msdp_data)
        self.log.debug(
            f"MSDP服务器状态子协商处理完毕，共收到{len(msdp_data.keys())}组VAR/VAL数据."
        )
    ...
    def handle_mssp(self, cmd):
        ...
        if cmd == WILL:
            ...
            if Settings.server["MSSP"]:
                ...
                self._mtts_index = 0
                self.log.debug("发送选项协商, 同意MSSP协商 IAC DO MSSP.等待子协商")
            else:
                self.session.write(IAC + DONT + MSSP)
                self.log.debug("发送选项协商, 不同意MSSP协商 IAC DONT MSSP.等待子协商")
        elif cmd == WONT:
            nohandle = True
        elif cmd == DO:
            nohandle = True
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的MSSP协商: IAC {name_command(cmd)} MSSP"
            )
    ...
    def handle_mssp_sb(self, data):
        ...
        self.log.debug(
            f"MSSP服务器状态子协商处理完毕，共收到{len(svrStatus.keys())}组VAR/VAL数据."
        )
    ...
    def handle_mccp2(self, cmd):
        ...
        if cmd == WILL:
            ...
            if Settings.server["MCCP2"]:
                ...
                self.log.debug("发送选项协商, 同意MCCP V2协商 IAC DO MCCP2")
            else:
                self.session.write(IAC + DONT + MCCP2)
                self.log.debug("发送选项协商, 不同意MCCP V2协商 IAC DONT MCCP2")
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的MCCP V2协商: IAC {name_command(cmd)} MCCP2"
            )
    ...
    def handle_mccp3(self, cmd):
        ...
        if cmd == WILL:
            ...
            if Settings.server["MCCP3"]:
                ...
                self.log.debug("发送选项协商, 同意MCCP V3协商 IAC DO MCCP3")
            else:
                self.session.write(IAC + DONT + MCCP3)
                self.log.debug("发送选项协商, 不同意MCCP V3协商 IAC DONT MCCP3")
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的MCCP V3协商: IAC {name_command(cmd)} MCCP3"
            )
    ...
    def handle_msp(self, cmd):
        ...
        if cmd == WILL:
            ...
            if Settings.server["MSP"]:
                ...
                self.log.debug("发送选项协商, 同意MSP协商 IAC DO MSP")
            else:
                self.session.write(IAC + DONT + MSP)
                self.log.debug("发送选项协商, 不同意MSP协商 IAC DONT MSP")
        ...
        if nohandle:
            self.log.warning(
                f"收到服务器的未处理的MSP协商: IAC {name_command(cmd)} MSP"
            )
    ...
    def handle_mxp(self, cmd):
        ...
        if cmd == WILL:
            if Settings.server["MXP"]:
                ...
                self.log.debug("发送选项协商, 同意MXP协商 IAC DO MXP")
            else:
                self.session.write(IAC + DONT + MXP)
                self.log.debug("发送选项协商, 不同意MXP协商 IAC DONT MXP")
