...
+ 问题修复: 修复如果变量的值带ANSI颜色代码，会无法打印的问题。
+ 问题修复：修复如果变量的键是数字类型，会无法正常打印的问题。
+ 感谢 @hhyzz 提供的问题BUG和修复代码 :)

## 0.22.7 (2026-07-12)

+ 问题修复: 修复 #{sessionname} 无法切换会话的问题。
+ 问题修复: 调整 modules.py 中变量命名，防止出现歧义。

## 0.22.6 (2026-05-15)

+ 问题修复: 修复当关闭所有会话后，状态窗口就再无法正常显示的问题。
+ 问题修复: 修复当存在未关闭记录器，退出程序会卡住或者报错的问题。
...
+ 功能新增: 针对应用退出时的资源释放和清理进行了完善，确保插件中申请的资源在应用退出时可以正常卸载。
