...
+ 功能修改: 修改会话刷新逻辑，只有本会话是当前会话（位于前台）时，本会话的收到数据时调用的方法 session.invalidate() 才会真正调用刷新。
+ 功能修改: 增加tmux判断，当明确判断本程序运行于tmux中，且处于后台时，所有会话在处理数据时都不进行刷新，仅1s一次保持刷新。
+ 注意事项: 对于在个人脚本中调用的 session.application.invalidate() 方法，建议改成 session.invalidate()。因为会话的刷新会判断前台，而application的刷新会强制刷新。
+ 功能新增: 增加一个非解析提示符"/"，当在命令行行首使用该提示符时，所有后续命令不进行任何解析，直接发送到服务器。该提示符可以通过 pymud.cfg 中的 noparser 进行覆盖。
+ 功能新增：新增 verbose 模式
+ 功能调整：在#var等命令执行之后，会调用del删除创建的临时列表，以加快内存释放
+ 问题修复: 修复在使用指定网络（代理或者指定接口）连接服务器，再断开后，直接使用#con不会使用原指定网络，而改为使用默认网络配置的问题。
